...
// NewRegexpEngine returns a pointer to a newly created regular expression filter.
func NewRegexpEngine(filters []string) (*regexpFilter, error) {
	f := &regexpFilter{
		re: []*regexp.Regexp{},
	}
...
func (f *regexpFilter) Match(u any) bool {
	str, ok := u.(string)
	if !ok {
		return false
	}
