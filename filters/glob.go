...
// NewGlobEngine returns a pointer to a newly created glob pattern filter.
func NewGlobEngine(filters []string) (*globFilter, error) {
	f := &globFilter{
		globs: []glob.Glob{},
	}
...
func (f *globFilter) Match(u any) bool {
	str, ok := u.(string)
	if !ok {
		return false
	}
